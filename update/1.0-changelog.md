...
2. 工作盆深度优化
3. 置物台现在被满箱的保险库吸的时候,如果上面有物品,占用会很高,需要补充懒加载逻辑
4. 目前,传送带在其上面进行加工时,由于数量的变化,导致一直活跃,这是正确的,但是好像漏斗也跟着一直活跃?有没有一种办法传送带活跃但漏斗懒加载(尤其是传送带上是一个一个物品处理时)
5. 动力锯可以仿照我的实验性分支那样,维护一个黑白名单,如果是黑名单,不管有没有nbt都可以直接返回,如果是白名单(不是完全没有配方(规避过滤干扰)),那么进行正常的缓存逻辑等
6. 注液器配方缓存
