...
| 交互 | 动力臂与部分方块实体交互检测。 |
| 流体 | 可配置的全局流体传输调度。 |
| 工厂仪表 *(Create 6.0.x)* | 仅对反复确认状态未变化、且独立满足条件的展示仪表进行监控退避。 |
| 无线红石 | 仅在发射强度未变化时跳过重复的无线网络传播。 |
...
CLT 添加了 **懒惰时钟**，可对单个支持的机械动力方块实体进行控制。它的设置面板有两条控制栏，
均使用可配置的百分比序列（默认 `0`、`25`、`50`、`75`、`100`）：

- **动态控制**：定义自适应退避的上限，为该机器配置最大延迟的百分比；实际间隔会随机器状态在此
  上限内变化。
- **强制控制**：定义一个固定间隔，同样为配置最大值的百分比；启用后会覆盖动态退避。
- **强制 `0%`**：让机器保持全速，并关闭该机器的 CLT 优化。

两条控制栏是二选一的策略，不会叠加。服务端配置决定时钟普通右键默认循环哪一条；设置面板可直接
选择任一条。客户端叠加提示和机械动力护目镜信息可展示支持机器的当前状态。
...
| `factory-gauge` *(Create 6.0.x)* | 工厂仪表稳定监控退避开关与最大刷新间隔。 |
| `arm` | 动力臂延迟、全速例外和弱懒惰目标。 |
| `lazytick-clock` | 两条控制栏的百分比循环序列，以及普通右键默认循环的控制栏。 |
...
工厂仪表的退避同样有严格边界：仅当展示仪表已启用筛选、已满足条件，且没有请求端、补货模式、
红石输入、未加载链路、待处理承诺或入站面板/链路时才会生效；状态变化后会立即重置退避并恢复正常
监控。无线红石优化也不会延迟强度变化：任何实际的信号变化仍按机械动力原本的网络路径传播。
